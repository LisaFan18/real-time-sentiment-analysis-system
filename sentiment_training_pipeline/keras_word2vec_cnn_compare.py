...
#EPOCHS = 4
EPOCHS_SIZE_LIST = [2, 4, 6, 8 ,10]
#EPOCHS_SIZE_LIST = [1]

loss_tr = np.zeros([len(EPOCHS_SIZE_LIST), LOOP_NUM], dtype=np.float32)
accuracy_tr = np.zeros([len(EPOCHS_SIZE_LIST), LOOP_NUM], dtype=np.float32)
loss_val = np.zeros([len(EPOCHS_SIZE_LIST), LOOP_NUM], dtype=np.float32)
accuracy_val = np.zeros([len(EPOCHS_SIZE_LIST), LOOP_NUM], dtype=np.float32)
loss_ts = np.zeros([len(EPOCHS_SIZE_LIST), LOOP_NUM], dtype=np.float32)
accuracy_ts = np.zeros([len(EPOCHS_SIZE_LIST), LOOP_NUM], dtype=np.float32)
time_tr = np.zeros([len(EPOCHS_SIZE_LIST), LOOP_NUM], dtype=np.float32)

j=0
for epoch_size in EPOCHS_SIZE_LIST:
#for train_size in TRAIN_SIZE_LIST:
    # print("*******************************train_size = %0.1f " %(train_size))
    print(" ******************************* epoch_size = %0.1f *******************************" %(epoch_size))
    ...
    for i in range(LOOP_NUM):
        ...
        model.fit(x_train_seq, y_train, batch_size=BATCH_SIZE, epochs=epoch_size, validation_data=(x_val_seq, y_val))
...
## save res
np.save(os.path.join(MODEL_PATH, "epoch_loss_tr.npy"), loss_tr)
np.save(os.path.join(MODEL_PATH, "epoch_accuracy_tr.npy"), accuracy_tr)
np.save(os.path.join(MODEL_PATH, "epoch_loss_val.npy"), loss_val)
np.save(os.path.join(MODEL_PATH, "epoch_accuracy_val.npy"), accuracy_val)
np.save(os.path.join(MODEL_PATH, "epoch_loss_ts.npy"), loss_ts)
np.save(os.path.join(MODEL_PATH, "epoch_accuracy_ts.npy"), accuracy_ts)
np.save(os.path.join(MODEL_PATH, "epoch_time_tr.npy"), time_tr)
